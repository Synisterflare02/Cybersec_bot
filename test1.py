...
prompt1 = f"With the given threat feed tell whether severity of the feed is Critical, Medium or Low:'{threat_feed}'"
response1 = generate_response(prompt1)
st.write(response1)
if threat_feed:
    # Determine the severity based on the threat feed
    if "critical" in response1:
        severity = "High"
    elif "Medium" in response1:
        severity = "Medium"
    else:
        severity = "Low"
